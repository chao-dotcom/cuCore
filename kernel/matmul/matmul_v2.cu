// cuCore/kernel/matmul/matmul_v2.cu
//
// A properly-pipelined, higher-arithmetic-intensity GEMM kernel addressing two
// real bottlenecks found in earlier versions:
//
// 1) Under-sized register tile (4x4): low compute:load ratio
// 2) "Double-buffering" that never actually overlapped latency (wrote to
//    shared memory before syncing, so the sync still waited on the slow path)
//
// Fixes:
...
//
// Note: Assumes N is a multiple of 128 (true for all sizes in the benchmark)
// so tile loads skip bounds checks. Production version would need masking.
